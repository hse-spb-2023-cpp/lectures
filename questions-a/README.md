...
## Шаблоны
* Шаблоны классов и функций: синтаксис объявления и определения
...
* Structured binding для пар, простых структур, массивов, со ссылкой.
  * Не было: как делать для своих структур, что на самом деле происходит внутри, что происходит с временными значениями, проблемы с `std::minmax`
* Поиск имён
  * Квалифицированный и неквалифицированный поиск, порядок обхода вложенных namespace
  * Отличие между `std::` и `::std::`
  * ADL (argument-dependent lookup) для операторов и функций (без тонкостей)
...
* `void*`: мотивация, примеры использования.
...
* Отличия `unique_ptr` / `shared_ptr` от обычного указателя, когда что использовать
* Создание `unique_ptr` / `shared_ptr`: какие есть конструкторы, что делает `make_unique` / `make_shared`
  * Проблема с `make_unique` / `make_shared` и приватными конструкторами
...
### 19. Правило пяти и move-семантика
...
* Агрегатная инициализация простых классов через `{}`
  ...
  * Не было: строгое определение "простого класса" (агрегата) и его отличия между стандартами C++11, C++14, C++17, C++20, C++23.
...
* Типичный заголовочный файл.
  ...
  * Опасность неявного включения, концепция include what you use, примеры из стандартной библиотеки вашего компилятора (упражнение `missing-include`).
...
* External linkage/internal linkage.
  ...
  * `static`, где применять
* Формулировка One Definition Rule
  * Что происходит с перегрузками функций
  * Использование `inline` для обхода ODR, требования к нескольким определениям
* Пример ошибок линковщика: multiple definition, undefined reference.
...
* Создание и уничтожение объектов со статическим временем жизни.
  ...
  * Гарантии про порядок создания
    * Не было: порядок уничтожения объектов.
...
* Решение: идиома construct on first use.
  * Не было: отличия решения с автоматическим временем жизни и с динамическим.

## 4x Наследование и динамический полиморфизм
### 40. Базовое наследование
* Совместимость ссылок и указателей
* Не было: расположение подобъектов в памяти и padding (пустое место для выравнивания)
...
* Пример использования CRTP (Curiously recurring template pattern)

### 42. Полиморфные классы
* Полиморфные классы, виртуальные функции, `override`/`final` для методов, вызовы виртуальных функций
  * Не было: отличия от hiding
...
* Создание и использование разделяемой библиотеки через Boost.DLL для системы плагинов на основе полиморфных классов (`08-231109/problem/03-boost-dll`)
...
* Producer-consumer
* Тонкости разделяемых библиотек
...
* Когда нужен const-qualifier у `operator()` при использовании с STL
...
* строгое определение rvalue, реализация `std::move`
